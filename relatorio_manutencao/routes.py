...
def baixar_zip(id_relatorio_manutencao: int, db: Session = Depends(get_db), _usuario=Depends(get_current_user)):
    ...
    return FileResponse(caminho, media_type="application/zip", filename=relatorio.nome_arquivo_original)
...
def obter_revisao(id_relatorio_manutencao: int, db: Session = Depends(get_db), _usuario=Depends(get_current_user)):
    ...
    return {"id_relatorio_manutencao": id_relatorio_manutencao, "data_referencia": relatorio.data_referencia, "observacao": relatorio.observacao, "fotos": [{"arquivo": foto.nome_arquivo_zip, "id_ativo": foto.id_ativo, "id_plano_item": foto.id_plano_item, "valor": foto.valor_medido or "", "status": foto.status_item, "observacao": foto.observacao or "", "incluir": foto.incluir, "confianca": foto.confianca} for foto in fotos]}
